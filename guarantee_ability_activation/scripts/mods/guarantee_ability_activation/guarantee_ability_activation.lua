...
local _input_hook = function(func, self, action_name)
    ...
    if (type_str == "boolean" and out == true) or (type_str == "number" and out == 1) then
        if 
            action_name == "combat_ability_pressed"
            or action_name == "combat_ability_hold"
            or action_name == "combat_ability_release"
        then
            mod.debug.print(action_name)
        end
        local num_charges = getCombatAbilityNumCharges()    
        if 
            action_name == "combat_ability_pressed" and num_charges ~= nil and num_charges > 0
        then
            setPromise()
        end
        if 
            action_name == "combat_ability_hold" and not mod:get("enable_combat_ability_hold")
        then
            return false
        end
    end
