    ...
    if unit then
        ...
        if visual_loadout_system then
            if action == "action_special" then
                if wieldable_component.special_active and wieldable_component.num_special_activations == 0 then
                    return
                end
...
local function _on_slot_wielded(self, slot_name)
    ...
    if slot_weapon ~= nil and slot_weapon.weapon_template ~= nil then
        ...
        if weapon_template.action_input_hierarchy.special_action or weapon_template.action_input_hierarchy.special_action_hold then
